...
laggTillSamtal.addEventListener('click', function() {
	...
	document.getElementById("samtals-längd").value = "";
	document.getElementById("distraktion-text").value = "";
});
...
function createTableRow(kategori) {
	...
	cell_tid.innerHTML = (new Date()).toISOString().slice(0,10).replace(/-/g,"");
	if (currentRow % 2 == 0) {
	...
	db.collection("samtal").add({
		...
		tid: (new Date()).toISOString().slice(0,10).replace(/-/g,"")
	})
...
function toggleRows(text) {
  ...
  tr = tabell.getElementsByTagName("tr");
	...
	for (i = 0; i < kategorier.length; i++) {
		if (clicked) {
			deactivateBadge(raknareText[i]);
		} else {
			activateBadge(raknareText[i]);
		}
	}
